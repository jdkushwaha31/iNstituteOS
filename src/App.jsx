...
export default function CoachingLedger() {
  ...
  // Lock the page's own scroll while the mobile drawer is open. Without
  // this, the drawer + backdrop just sit visually on top of the page —
  // they don't stop it from scrolling underneath, so a drag anywhere
  // over the open drawer scrolls the dashboard behind it instead of
  // staying put. Restored on close and on unmount so it never gets left
  // stuck if the component unmounts while the drawer happens to be open.
  useEffect(() => {
    if (mobileNavOpen) {
